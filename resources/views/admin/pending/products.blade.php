...
@section('content')
    <div class="container py-5">
                    ...
                    <div class="col-md-4 mb-4">
                        <div class="card h-100">
                            ...
                            <div class="card-body">
                                ...
                                <p class="card-text">Nhà cung cấp:
                                    {{ $product->supplier ? $product->supplier->name : 'Chưa rõ' }}</p>
        ...
        <div class="card-body">
            <h5>Sản phẩm đã phê duyệt gần đây</h5>
                            ...
                            <div class="card h-100">
                                @if ($product->image)
                                    <img src="{{ Storage::url($product->image) }}" class="card-img-top"
                                        alt="{{ $product->name }}" style="max-height: 150px; object-fit: cover;"
                                        onerror="this.src='https://via.placeholder.com/150x150?text=No+Image'">
                                @else
                                    <img src="https://via.placeholder.com/150x150?text=No+Image" class="card-img-top"
                                        alt="{{ $product->name }}" style="max-height: 150px; object-fit: cover;">
                                @endif
                                <div class="card-body">
                                    <h6 class="card-title">{{ $product->name }}</h6>
                                    <p class="card-text">Giá: ₫{{ number_format($product->regular_price, 0, ',', '.') }}
                                    </p>
                                    <p class="card-text">Tồn kho:
                                        {{ $product->inventory ? $product->inventory->stock : 0 }}</p>
                                </div>
                            </div>
                        </div>
                    @endforeach
            ...
            @else
                <p class="text-center">Chưa có sản phẩm nào được phê duyệt gần đây.</p>
            @endif
        </div>
    </div>
@endsection
